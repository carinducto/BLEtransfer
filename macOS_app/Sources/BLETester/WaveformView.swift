...
struct WaveformView: View {
    ...
    private var waveformInfo: some View {
        HStack(spacing: 24) {
            infoItem("Block", String(waveform.header.blockNumber))
            infoItem("Sample Rate", "\(waveform.header.sampleRateHz / 1_000_000) MHz")
            infoItem("Samples", String(waveform.header.sampleCount))
            infoItem("Carrier", "\(waveform.header.pulseFreqHz / 1_000_000) MHz")
            infoItem("Temp", String(format: "%.1f°C", Float(waveform.header.temperatureCx10) / 10.0))
            infoItem("Gain", "\(waveform.header.gainDb) dB")
        }
        .font(.caption)
        .padding(.horizontal)
    }
    ...
    private func drawWaveform(context: GraphicsContext, size: CGSize) {
        ...
        // Calculate scaling
        let minSample = waveform.samples.min() ?? 0
        let maxSample = waveform.samples.max() ?? 1
        ...
        for (index, sample) in waveform.samples.enumerated() {
            let x = margin + (plotWidth * CGFloat(index)) / CGFloat(waveform.samples.count - 1)
        ...
        if waveform.header.triggerSample < waveform.samples.count {
            let triggerX = margin + (plotWidth * CGFloat(waveform.header.triggerSample)) / CGFloat(waveform.samples.count - 1)
            context.stroke(
                Path { path in
        ...
        let timeScalePoints = [0, waveform.samples.count / 4, waveform.samples.count / 2, waveform.samples.count * 3 / 4, waveform.samples.count - 1]
        for point in timeScalePoints {
            let x = margin + (plotWidth * CGFloat(point)) / CGFloat(waveform.samples.count - 1)
            let timeUs = Float(point) / Float(waveform.header.sampleRateHz) * 1_000_000
